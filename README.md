...
Create a product in the Dodo dashboard, then set `DODO_PRODUCT_ID`. Add a webhook for `payment.succeeded` → `https://your-domain.com/api/dodo/webhook` and copy the **Signing secret** into `DODO_PAYMENTS_WEBHOOK_KEY` (or `DODO_WEBHOOK_SECRET`) in Vercel.
